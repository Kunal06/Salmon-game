...
bool Box::init()
{
	...
	{
		if (!box_texture.load_from_file(textures_path("white_box.png")))
		{
			fprintf(stderr, "Failed to load box texture!");
			return false;
		}
	}

	// The position corresponds to the center of the texture
	float wr = box_texture.width * 0.5f;
...
void Box::set_rotation(float radians)
{
	motion.radians = radians;
}

void Box::rotate(float off)
{
	// move in direction
...
void Box::angled_move(float off)
{
...
vec2 Box::get_bounding_box() const
{
...
// Set box movement flag (used to keep track of the action and release(f) of the user keys)
void Box::set_movement(const std::string &flag)
{
	if (flag == "up")
	{
		is_up = true;
...
// Get box movement flag (used to keep track of the movement)
bool Box::get_movement(const std::string &flag)
{
	if (flag == "up")
	{
		return is_up;
...
// Called when the box collides with a turtle or shark
void Box::kill()
{
	m_is_alive = false;
}

void Box::set_debug_mode(bool value)
{
	debug_mode = value;
}

void Box::set_box_position(vec2 off)
{
	motion.position.x = off.x;
	motion.position.y = off.y;
}
